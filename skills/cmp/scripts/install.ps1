...
function Main {
  ...
  if (Get-Command cmp -ErrorAction SilentlyContinue) {
    Write-Host "[OK] cmp is available"
    return 0
  }
  ...
  switch ($platform) {
    ...
    "macos" {
      if (Get-Command brew -ErrorAction SilentlyContinue) {
        & brew install cmp 2>$null
        Write-Host "[OK] cmp installed"
      } else {
