...
usage() {
  cat <<'USAGE'
Usage: aws-diagnostics.sh [--json] [--out <file>]

Runs aws preflight, auth, and context checks and prints a combined report.
USAGE
}
...
preflight_output="$($script_dir/aws-preflight.sh 2>&1)"
preflight_rc=$?
auth_output="$($script_dir/aws-auth-status.sh 2>&1)"
auth_rc=$?
context_output="$($script_dir/aws-context.sh 2>&1)"
context_rc=$?
