...
usage() {
  cat <<'USAGE'
Usage: glab-diagnostics.sh [--json] [--out <file>]

Runs glab preflight, auth, and context checks and prints a combined report.
USAGE
}
...
preflight_output="$($script_dir/glab-preflight.sh 2>&1)"
preflight_rc=$?
auth_output="$($script_dir/glab-auth-status.sh 2>&1)"
auth_rc=$?
context_output="$($script_dir/glab-context.sh 2>&1)"
context_rc=$?
