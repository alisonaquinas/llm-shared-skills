...
Testing collects evidence that the workflow behaves correctly before integration.

1. Exercise the happy path and one meaningful variant.
2. Exercise one failure path and one recovery path.
3. Keep fixtures disposable and deterministic.
4. Record expected outputs and observed results.
5. Prefer dry runs when the workflow can mutate state.
...
9. A workflow without negative-path evidence is not ready for verification.
